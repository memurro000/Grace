...
namespace Grace::integration::functional {
    ...
    void linear_combination(
        vector_inout result, vector_in a, num_t multiplier, vector_in b
    ) {
        Kokkos::parallel_for(
            "linear_combination", a.extent(0),
            KOKKOS_LAMBDA(const size_t i) {
                result(i) = a(i) + multiplier * b(i);
            }
        );
    }

    inline 
    vector_out linear_combination(
        vector_in a, num_t multiplier, vector_in b
    ) {
    ...
    inline 
    void reconsider_solution(
        vector_inout result,
        vector_in prev_solution, 
        vector_in k1, vector_in k2,
        vector_in k3, vector_in k4,
        num_t dt
    ) {
        const num_t dt_fract_6 = dt / 6.0;
        Kokkos::parallel_for(
            "reconsider_solution", prev_solution.extent(0),
            KOKKOS_LAMBDA(const size_t i) {
                result(i) = prev_solution(i) +
                    dt_fract_6 * (k1(i) + 2.0 * (k2(i)   +   k3(i)) + k4(i));
            }
        );
    }

    inline 
    vector_out reconsider_solution(
        vector_in prev_solution, 
        vector_in k1, vector_in k2,
        vector_in k3, vector_in k4,
        num_t dt
