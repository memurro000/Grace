...
inline void fill(vector_t & vector, num_t with) {
    Kokkos::parallel_for(
        "zero_system_for1", vector.extent(0), KOKKOS_LAMBDA(const size_t i) { vector(i) = with; });
}

inline vector_t make(const std::string & info, size_t n, num_t filling = num_t{ 0 }) {
...
class logistic_system {
  public:
    logistic_system(num_t alpha, num_t carrying_capacity) : _alpha(alpha), _K(carrying_capacity) {
        assert(alpha >= 0.0);
    ...
    vector_t operator()(num_t t, const vector_t & y) {
        ...
        Kokkos::parallel_for(
            "logistic_derivative_compute", y.extent(0),
            KOKKOS_LAMBDA(const int i) { result(i) = alpha * y(i) * (1.0 - y(i) / K); });
...
concept analytical_solution = requires(Callable callable, vector_t y_0, num_t t_0, num_t t_end) {
...
inline vector_t zero_system_analytical(const vector_t & y_0, num_t t_0, num_t t_end) {
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(const size_t i) { result(i) = y_0(i); });
    return result;
}
  ...
  public:
    ...
    vector_t operator()(const vector_t & y_0, num_t t_0, num_t t_end) {
        ...
        Kokkos::parallel_for(
            ...
            KOKKOS_LAMBDA(const size_t i) { result(i) = y_0(i) + filling * (t_end - t_0); });
        return result;
    }
...
class logistic_system_analytical {
  public:
    ...
    vector_t operator()(const vector_t & y_0, num_t t_0, num_t t_end) {
        ...
        Kokkos::parallel_for(
            "logistic_system_analytical", y_0.extent(0), KOKKOS_LAMBDA(const size_t i) {
