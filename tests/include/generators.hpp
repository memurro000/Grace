...
inline num_t index_step2(size_t i) {
    if (i % 2 == 0) return i;
    else
        return -i;
}
...
inline generator_function_t shifted(const generator_function_t & generator, num_t shift) {
    return [&](size_t i) { return shift + generator(i); };
}
